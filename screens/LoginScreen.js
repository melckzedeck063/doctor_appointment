  ...
  return (
    <View style={{height : responsiveHeight(100)}} className="bg-cyan-600">
      ...
      <Text className="text-2xl font-medium text-cyan-600 text-center" >Sign In</Text>
          <View className="my-2">
           <Text className={`text-cyan-600 text-xl ${Platform.select({android : 'text-lg'})}`} > Username</Text>
      <Controller
        control={control}
        defaultValue =  ""
        rules={{
      ...
      { errors.username && <Text className="text-red-400" > {errors.username.message} </Text>}
       ...
       <View className="my-2">
         <Text className={`text-cyan-600 text-xl ${Platform.select({android : 'text-lg'})}`} > Password</Text>
         <Controller
        control={control}
        rules={{
        ...
        render={({ field: { onChange, onBlur, value } }) => (
          <TextInput  className={`rounded-md text-slate-800 bg-gray-200 px-4 py-2.5 ${Platform.select({android : 'py-1.5'})} ${errors.password? 'border-2 border-red-400' : 'border-2 border-slate-300'}`}
      ...
      {errors.password && <Text className="text-red-400"> {errors.password.message} </Text>}
        ...
        <View>
            ...
            <Text className="font-medium text-slate-700 text-center" >Don't have an account ? </Text>
             ...
             >
                <Text className="text-xl font-medium text-cyan-600 text-center" >Sign Up</Text>
             </TouchableOpacity>
            </View>
        </View>
      </View>
