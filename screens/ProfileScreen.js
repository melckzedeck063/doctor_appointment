...
import image2 from '../assets/images/pexels-shvets-production-8413184.jpg';
...
const ProfileScreen = () => {
    ...
    const { register, reset, control, handleSubmit, formState: { errors, isDirty, isValid } } = useForm({
        defaultValues :  {
          user_role : "user",
          username : 'melckzedeck063@gmail.com',
          firstName : 'melckzedeck',
          lastName : 'james',
          telephone : '0744219981'
        },
          mode: 'all',
      })
  ...
  return (
   ...
   <View className={`w-full h-full bg-slatee-700 mt-3 ${Platform.select({ios : 'py-12', android : 'py-12'})}`}>
    ...
    <View className="">
           ...
           <Text className="text-2xl font-medium text-cyan-600 text-center my-2" >My Profile</Text> 
          </View>
       ...
       <Text className="text-lg text-slate-600" >FirstName</Text>
        ...
        render={({ field: { onChange, onBlur, value } }) => (
          <TextInput  className={`rounded-md bg-slate-100 px-4 py-3 capitalize ${errors.firstName? 'border-2 border-cyan-600' : ''}`}
