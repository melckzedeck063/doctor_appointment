...
//   const {params  :  {props}  }  = useRoute();
  ...
  return (
    <StripeProvider publishableKey="your_publishable_key">
      <SafeAreaView  />
        <Text className={`text-cyan-600 font-bold text-center text-2xl my-2 ${Platform.select({android : 'text-xl'})}`} >Checkout</Text>
      <View style={[styles.card]} className={`p-1 rounded-lg ${Platform.select({android : 'mt-8'})}`} >
        ...
        <TouchableOpacity  style={{alignSelf : 'center'}}
         onPress={handleCheckout} className="bg-cyan-600 rounded-lg w-6/12">
         <Text style={{fontSize : responsiveFontSize(2.5)}} className={`text-white font-medium text-center  ${Platform.select({android : 'text-lg'})}`}>Save Details</Text>
        </TouchableOpacity>
      </View>
    </StripeProvider>
  );
